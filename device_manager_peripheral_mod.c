...
void dm_ble_evt_handler(ble_evt_t * p_ble_evt)
{
    ...
    switch (p_ble_evt->header.evt_id)
    {
        ...
        case BLE_GAP_EVT_SEC_PARAMS_REQUEST:
			...
			if (failed_auth >= NO_BOND_FAILED) {																									//This is a modification to the original device_manager
				err_code = sd_ble_gap_disconnect(p_ble_evt->evt.gap_evt.conn_handle, BLE_HCI_REMOTE_USER_TERMINATED_CONNECTION);	  //If the number of failed attempts is >= 3 reject all bonding
				APP_ERROR_CHECK(err_code);																							//Already bonded should still work
				break;
			}
